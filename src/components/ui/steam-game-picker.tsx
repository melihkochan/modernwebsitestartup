    ...
    <div ref={dropdownRef} className={cn("relative w-full", className)}>
      <div className="relative">
        <input
          ...
          onFocus={() => {
            if (displayedResults.length > 0) setIsOpen(true);
          }}
      ...
      {isOpen && (displayedResults.length > 0 || isLoading) && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-lg shadow-[var(--shadow-md)] z-50 max-h-60 overflow-y-auto py-1">
          {isLoading ? (
            <div className="px-4 py-3 text-xs text-zinc-500 animate-pulse flex items-center gap-2">
            ...
            displayedResults.map((game) => (
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2 hover:bg-[var(--bg-overlay)] transition-colors text-left border-none cursor-pointer"
              >
